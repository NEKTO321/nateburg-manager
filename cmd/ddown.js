...
module.exports = {
    name: 'мз-понизить', // name of the command
    ...
    run: async(client, message, args) => {
        ...
        if(dzvan === 0) {
            ...
            .setDescription(`${user} понижен (МЗ)`)// if there is reason provided then show the reason, if not then show 'No reson provided'
            .setColor('#FF0000')
            client.channels.cache.get('1040975520822738994').send({embeds: [embed]})
            let dm = new MessageEmbed()
            .setDescription(`Поздравляю!\nВы понижены, ваша новая роль: Медсестра/Медбрат (МЗ)`)
            .setColor('#f1c40f')
            user.send({ embeds: [dm] })
